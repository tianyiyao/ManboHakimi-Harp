...
> 另外新增崩溃日志（`config/harpguide.log`，出问题发给作者即可定位）。
>
> **v0.14.1**：判定手感重做。修掉「第一次没按好、再按还是 MISS」「长按整条判 MISS」「键缝空按也断连击」三个残留问题；预览图的中文渲染也一并修好了（离屏渲染补上中文字体注册）。
>
> **v0.14**：曲目列表从设置面板搬到浮窗**左侧的悬停抽屉**，鼠标贴上去就展开、移开自动缩回，收起后仍留一条把手。
>
...
程序异常的痕迹都留在 **`config/harpguide.log`**（与 `settings.json` 同目录，便携模式下就在 EXE 旁边）：

- 每次启动留一行：版本、是否打包、数据目录 —— 排查「设置/曲目去哪了」先看这行；
- 一旦崩溃（含点按钮时崩）会追加完整调用栈 + 版本 + Python 环境；超过 512KB 自动轮转成 `.log.1`，不会无限变大。

**反馈问题时把这个文件发过来即可**，不用截图描述。程序不联网、不上报，日志只存在你自己机器上。
...
曲目列表不再是「点设置按钮才弹出来」，而是**贴在浮窗左边缘的抽屉**：

| 状态 | 表现 | 怎么触发 |
|------|------|----------|
| 缩进 | 只露出 22px 宽的竖条把手（带竖排「曲目」字样、曲目数量、方向箭头） | 默认状态，鼠标移开后自动回到这里 |
| 展开 | 滑出完整曲目列表，可滚动 | 鼠标移到把手上（移上去立刻展开） |

- 移开鼠标 **450ms** 后自动缩回；缩回前会用光标真实位置复核一次，鼠标蹭过不会抽搐
- 点选曲目后自动收回，选完就走
- 在列表项上**右键**：在编辑器中编辑 / 重命名 / 删除（仅用户曲目）
- 把手纵向范围**上下都避开**：上面不挡顶部信息条（照样能拖窗口、点齿轮），下面不挡底部热键提示条
- 开启**鼠标穿透**或进入**琴键校准**时抽屉会自动撤下（穿透态收不到鼠标事件，留着会让人以为卡住）
- 把手所在的左边缘让给抽屉（否则鼠标会被当成压在窗口边框上），拖拽缩放请用其他三边
- 设置面板里保留了一行指路说明，功能参数不受影响

> 抽屉展开时会临时盖住左侧的内容（瀑布流/琴键/进度条左段），缩回后完全让开——这是抽屉的常规行为，选曲时人本来也没在弹。
...
### 判定手感（v0.12 起，v0.14.1 重做）

核心原则：**这个工具是练习器，不是裁判。** 按错了要提醒，但不能罚得让人不敢练。
...
| `GREAT` | ±220ms | 绿色 |
| `GOOD` | ±350ms | 青色 |
| 抢拍/拖拍 | 350 ~ 510ms | **记一次 MISS，但不消耗音符**——可以在窗口内重按救回来 |
| 漏音 | 音符起点 +510ms 仍无人按 | `MISS`，音符作废 |
| **键缝空按** | 该键附近完全没有音符 | **不判 MISS、不断连击**（试键、摸键位是常态） |

长按（hold）单独判定，见下表。

### 判定手感修复史

**v0.12（第一次修「按对了还 MISS」）**

| 问题 | 修复 |
|---|---|
| 判定窗口偏严（±300ms 就到顶） | 放宽为 **±110 / ±220 / ±350ms** |
| 一次失误被判**两个** MISS（按键空了记一次，音符滑过再记一次） | 新增「匹配窗口」：这一列附近有音符时，迟到/重复的按键**静默忽略**，不叠加 MISS |
| 玩家整体按早/按晚（人的反应 + 画面延迟） | 新增**判定偏移**，可把整个判定基准平移对齐 |
| 游戏满载时帧被拖慢，快速轻敲可能被漏读 | 轮询同时读取 `GetAsyncKeyState` 的按下标志位，**不漏掉帧间的轻敲** |
| 漏音定时器与判定窗口一样宽，会**抢先**把"还能判 GOOD"的音符判成 MISS | 漏音窗口 = 判定窗口 **+ 宽限**（v0.14.1 起统一为 `GRACE_MS = 160ms`） |
...
| 问题 | 根因 | 修复 |
|---|---|---|
| 第一次没按好，**再按还是 MISS** | 旧逻辑：偏移超出 GOOD 窗口就直接把音符吃掉。重按时匹配不到原音符，又因为附近有音符被静默忽略——玩家看到的是「第一次 MISS，第二次还是 MISS」，救不回来 | 拆成两档：**GOOD 窗口内**才消耗音符；**GOOD~510ms 宽容带内**只记一次 MISS、**不消耗音符**，窗口内重按仍可正常判定。同一音符只提醒一次，连按不刷屏 |
| **长按整条判 MISS** | 旧逻辑只看「按键边沿」。而长音的正确手法是**在起点之前就压住**，那一刻没有新边沿，于是整条长音被漏音判成 MISS | 长音改为「**键处于按住状态** + 与音符区间有重叠」判定：早按到 300ms 前都算有效（提前压住是正确手法，偏移按 0 计即 PERFECT），晚按给 120ms 余量、至少算 GOOD。为此 `poll()` 额外返回「当前按住集合」 |
...
- **每一遍独立计分**：循环回到 A 点时连击与得分清零，重新开始数（这样才知道这一遍比上一遍好在哪）；
  同一个循环内按键判定照常生效，不会出现"第二遍按了不给分"
- A 点之前的音符不算这一遍的漏音（它们不在练习范围里，不会平白刷 MISS）
...
| 演奏 | 变速滑杆（0.25x–2x）、整曲循环开关、**预备拍数**（0–8）、**长按预亮提前量**、自动变调、命中反馈、**判定偏移** |
| 显示 | 不透明度、瀑布流高度、**下落时间**（毫秒）、拍线开关、**歌词开关**、**底部热键提示条**、长按流光开关、松开预警开关 |
| 鼠标穿透 | 全局穿透开关 |
| 热键冲突 | 启动时检测，被占用时面板顶部出现告警条列出冲突项 |

> **v0.14 起曲目列表已迁到左侧边栏**（见上文「曲目侧边栏」），面板里只留一行指路说明。
...
│   ├── waterfall.py      音符瀑布流渲染（长条/流光/橙色预警/列对齐 + 判定浮层）
│   ├── keys.py           琴键提示区（七态状态机 + 校准编辑交互 + 命中光环）
│   ├── keywatch.py       被动按键轮询（GetAsyncKeyState，判定/命中反馈用）
│   ├── sidebar.py        曲目侧边栏（悬停展开的左侧抽屉）
│   ├── hintbar.py        底部热键提示条（按宽度自适应换行）
│   ├── lyrics.py         歌词行提示
...
├── preview/              界面截图（README 引用）
├── preview_*.py          离屏渲染验证脚本（截图 / 反馈 / 侧边栏）
